...
group.add_argument('--checkpoint-hist', type=int, default=1000, metavar='N',
                   help='number of checkpoints to keep (default: 1000)')
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if update_idx % args.log_interval == 0:
            ...
            if utils.is_primary(args):
                total_norm = sum([w.grad.detach().norm() ** 2 for w in model_parameters(model, exclude_head='agc' in args.clip_mode) if w.grad is not None])
                if total_norm > 0:
